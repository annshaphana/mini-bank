...
def customer_info():
    ...
    userpassword=input("Enter user_password:")
...
def create_next_customer_id():
    if not os.path.exists("customers.txt"):
        ...
        with open("customer.txt",'r')as file:
            return f'c(int(file.readline()))'

def create_customer():
    customer=customer_info()
    create_next_customers_id= create_next_customers_id()
    with open ("customers.txt",'a')as customers_file,open("users.txt",'a')as users_file:
        customers_file.write(f'{customer[0]}/t{customer[1]}\n')
        users_file.write(f'{customer[2]}/t{customer[3]}/t')
import os
